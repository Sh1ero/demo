...
## ✔️ Задание 1 <code>[ SAMBA ]</code>
### Произведите `базовую` настройку устройств

### ISP
```
apt install samba-ad-dc krb5-user winbind -y
```
```
...
```
```
Realm: AU-TEAM.IRPO (заглавными буквами)
Domain: AU-TEAM
Server Role: dc
...
```
```
apt install samba-common-bin krb5-user winbind
```
```
...
### Создадим разделы для sdb и sdc (G N ener W)
```
fdisk /dev/sdb
```
```
fdisk /dev/sdc
```
### Создайте RAID 0 массив
```
sudo mdadm --create /dev/md0 --level=0 --raid-devices=2 /dev/sdb /dev/sdc
```

### Теперь можно приступить к созданию файловой системы. Для начала отформатируем разделы в ext4: 
```
mkfs.ext4 /dev/md0
```

### Добавьте в автозагрузку (fstab)

```
echo "UUID=$(sudo blkid -s UUID -o value /dev/md0p1) /mnt/raid0 ext4 defaults 0 0" | sudo tee -a /etc/fstab
```
### ПРОВЕРКА
```
...
### Для начала на hq-rtr нужно установить chrony

```
apt install chrony
```

# Заходим в файл /etc/chrony/chrony.conf 
...
allow 0/0
```
# Перезапускаем chriny

```
systemcl restart chronyd
```
# На каждую машину кроме isp устанавливаем chrony 
```
apt install chrony
```
# И прописываем 
...
ssh-copy-id -p 2026 locadm@192.168.2.3
ssh-copy-id -p 2026 net_admin@172.16.1.2
ssh-copy-id -p 2026 net_admin@172.16.2.2
```
<br/>
...
## ✔️ Задание 11 `[ZЯндекс браузер]`

## HQ-CLI

- Установите необходимые зависимости ( хз надо не надо но пусть будет)

```
sudo apt-get install libappindicator1 libindicator7 ca-certificates
```

- Скачайте deb-пакет с официального сайта с помощью wget
```
wget -O yandex-browser.deb https://browser.yandex.ru/download/?banerid=6302000000&zih=1&beta=1&os=linux&x64=1&package=deb&full=1
```
